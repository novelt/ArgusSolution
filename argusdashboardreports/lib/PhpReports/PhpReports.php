...
class PhpReports {
	...
	public static function convertDashboardToPdf($dashboard, $contentDisposition, $title) {
		//$content = self::getDashboard($dashboard);
		$pdfConvert = new PdfConverter();
		$pdfConvert->convertDashboard($dashboard, $contentDisposition, $title);
	}

	/**
	 * To separate Dashboard online and Printable pdf Dashboard
	 *
	 * @param $dashboard
	 * @throws Exception
	 */
	public static function displayDashboardPdf($dashboard){
		$content = self::getDashboard($dashboard);

		echo self::render('html/dashboardPdf',array(
			'dashboard'=>$content
		));
	}
