...
Flight::route('/PdfConvDashboard/@name',function($name) {
    $contentDisposition = $_GET['contentDisposition'];
    $title = $_GET['title'];
    PhpReports::convertDashboardToPdf($name, $contentDisposition, $title);
});

Flight::route('/dashboardPdf/@name',function($name) {
    PhpReports::displayDashboardPdf($name);
});
