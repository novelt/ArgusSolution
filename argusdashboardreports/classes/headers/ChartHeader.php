...
class ChartHeader extends HeaderBase {
	static $validation = array(
		...
		// Change Chart from Column to line based on number of rows
		'columnToLine' => array(
			'default'=>0
		),
		// Enabled column chart labels
		'enabledLabels' => array(
			'default'=>false
		),
		'type'=>array(
	...
	public static function init($params, &$report) {
		...
		// TODO : This hack allow defining dynamic Chart Header and replace chart present in the php file.
		// 		If this header is not present in the php file as header, Chart dynamic header does'nt work
		if (isset($params['title']) && $params['title'] != 'OVERRIDDEN') {
	...
	public static function parseShortcut($value) {
		...
		foreach($params as $param) {
			...
			if(strpos($param,'=') !== false) {							
				...
				if(in_array($key,array('x','y','colors','dashStyle'),true)) {
					$val = explode(':',$val);
				}
			}
			else {
