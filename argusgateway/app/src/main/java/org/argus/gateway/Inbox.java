...
public class Inbox {
    ...
    public synchronized void forwardMessage(IncomingMessage message) {
        ...
        if (incomingMessages.containsKey(uri)) {
            app.log("Duplicate incoming " + message.getDisplayType() + "  : " + message.getDescription() + ", skipping");
            return;
        }

        incomingMessages.put(uri, message);
        app.log("Received "+ message.getDisplayType() + " : " + message.getDescription());
    ...
    {        
        ...
        if (message.scheduleRetry(0)) // pass 0 to never give up the retry
        {
